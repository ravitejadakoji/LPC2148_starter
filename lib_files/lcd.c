...
uint8_t v_LCDTrackRow;
uint8_t v_LCDTrackCur;
uint8_t LCDRowNumAdd[]={0x80,0xC0,0x90,0xD0};
...
{
    LCDConfig.v_maxSuppRows = var_LCDNoOfRows;
    LCDConfig.v_maxSuppChars = var_maxCharsPerRow;
    
    if(var_LCDNoOfRows > C_LCDRowTwo)
    {
        LCDRowNumAdd[C_LCDRowTwo] = 0x90 + (var_maxCharsPerRow & 0x0Fu);
        LCDRowNumAdd[C_LCDRowThree] = 0xD0 + (var_maxCharsPerRow & 0x0Fu);
    }
    delay_ms(100);
...
{
    if(var_RowNum < LCDConfig.v_maxSuppRows)
    {
        v_LCDTrackCur = 0x00;
        v_LCDTrackRow = var_RowNum;
        LCD_CMDWrite(LCDRowNumAdd[var_RowNum]);
    }
}
...
void LCD_GoToNextRow(void)
{
    ...
    if(v_LCDTrackRow >= LCDConfig.v_maxSuppRows)
    {
        v_LCDTrackRow = C_LCDRowZero;
    }
    
    LCD_CMDWrite(LCDRowNumAdd[v_LCDTrackRow]);
}
...
{
    if((var_RowNum<LCDConfig.v_maxSuppRows) && (var_charNum<LCDConfig.v_maxSuppChars))
    {
        v_LCDTrackCur = var_charNum;
        v_LCDTrackRow = var_RowNum;
        LCD_CMDWrite(LCDRowNumAdd[var_RowNum]+var_charNum);
    }
}
#endif
...
/*****************************************************************************/
	...
	void LCD_ScrollMsg(uint8_t var_RowNum, char *ptr_msgPtr)
	{
		...
		if(var_RowNum >= LCDConfig.v_maxSuppRows)
			var_RowNum = C_LCDRowZero;
		
		LCD_CMDWrite(CMD_LCD_DISPLAY_ON_CURSOR_OFF);
		
		for(i=0;ptr_msgPtr[i];i++)
		{
			LCD_GoToRow(var_RowNum);
